...
2. `agent start <name>` → `docker run -d` with selective bind mounts + SSH/GPG agent sockets + host user env vars
...
- **`~/.gitconfig` is not mounted by default.** If git operations need the host user identity or signing config, add it via `--mount ~/.gitconfig` or `AGENT_EXTRA_MOUNTS`.
